...
export default function ReviewClient({ initialReviews }: { initialReviews: any[] }) {
  ...
  // Jika data dari database masih kosong, pakai data dummy agar desain tidak rusak
  const reviews = initialReviews.length > 0 ? initialReviews : [
    { id: "1", nama: "Belum Ada Ulasan", peran: "Sistem", pesan: "Belum ada testimoni dari pelanggan.", rating: 5 },
    { id: "2", nama: "Belum Ada Ulasan", peran: "Sistem", pesan: "Belum ada testimoni dari pelanggan.", rating: 5 },
  ];

  // Duplikasi array jika datanya terlalu sedikit agar slider tidak putus di tengah jalan
  const safeReviews = reviews.length < 4 ? [...reviews, ...reviews, ...reviews] : reviews;
      ...
      <div className="w-full mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10 md:mb-16 px-5">
          ...
          <p className="text-gray-500 text-sm md:text-[16px] leading-relaxed">
            Kepercayaan dan kepuasan Anda adalah prioritas utama kami. Berikut adalah pengalaman para pelanggan yang telah bekerja sama dengan Bali Limestone.
          </p>
        </div>
        ...
        >
          ...
          >
            {/* SET 1 */}
            <div ref={measureRef} className="flex gap-4 md:gap-6 pr-4 md:pr-6 shrink-0 px-4 md:px-0">
              {safeReviews.map((item, idx) => (
                <TestimonialCard key={`set1-${item.id}-${idx}`} item={item} />
              ))}
            </div>
            ...
            <div className="flex gap-4 md:gap-6 pr-4 md:pr-6 shrink-0 px-4 md:px-0">
              {safeReviews.map((item, idx) => (
                <TestimonialCard key={`set2-${item.id}-${idx}`} item={item} />
              ))}
            </div>
          </motion.div>
        </div>
    ...
    <div className="w-[280px] sm:w-[320px] md:w-[400px] bg-[#fafafa] rounded-2xl p-5 md:p-8 flex flex-col justify-between shrink-0 border border-gray-100 transition-all duration-300 hover:border-gray-300">
      <div className="flex justify-between items-start mb-5 md:mb-6 gap-3">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <h4 className="text-base md:text-lg font-medium text-black">{item.nama}</h4>
        ...
        <div className="flex gap-1 shrink-0">
          {[...Array(5)].map((_, index) => (
            <svg
              key={index}
              className={`w-4 h-4 md:w-5 md:h-5 ${index < item.rating ? "text-[#ffcc00]" : "text-gray-300"}`}
              fill="currentColor"
              viewBox="0 0 24 24"
            >
