...
export default function ServicesPage() {
  ...
  return (
    <main className="w-full bg-black">
      ...
      {/*  MOBILE vertikal ── */}
      <section className="sm:hidden min-h-screen flex flex-col pt-24">
        {services.map((service, index) => {
          const isActive = active === index;
          ...
          return (
            <div
              ...
              style={{ minHeight: isActive ? "280px" : "80px" }}
              onClick={() => setActive(isActive ? null : index)}
            >
              {/* Background */}
              <div
              ...
              <div className="relative z-10 flex flex-col justify-between h-full p-6">
                <div>
                  ...
                  <h2 className={`text-white font-bold leading-tight transition-all duration-500
                    ${isActive ? "text-3xl text-yellow-400" : "text-xl"}`}>
                    {service.title}
                  </h2>
                ...
                {/* Lihat Selengkapnya */}
                <div className={`overflow-hidden transition-all duration-500
                  ${isActive ? "max-h-10 opacity-100 mt-4" : "max-h-0 opacity-0"}`}>
                  <Link
                    href={`/services/${service.slug}`}
                    onClick={(e) => e.stopPropagation()}
                    className="flex items-center gap-3 text-white text-sm"
